...
namespace Managed.Adb
{
	...
	public class ListingServiceReceiver : MultiLineReceiver
	{
		...
		/// 
		/// </summary>
		private const string LOG_TAG = "ListingServiceReceiver";
		...
		protected override void ProcessNewLines(string[] lines)
		{
			foreach (string line in lines)
			{
				...
				if (!m.Success)
				{
					Log.D(LOG_TAG, "not match: {0}", line);
					continue;
				}
				// get the name
				...
				long.TryParse(string.IsNullOrEmpty(sizeData) ? "0" : sizeData, out long size);
				string date1 = m.Groups[5].Value.Trim();
				string date2 = m.Groups[6].Value.Trim();
				string date3 = m.Groups[7].Value.Trim();

				DateTime date = DateTimeHelper.GetEpoch();
				...
				if (date1.Length == 3)
				{
					...
					date = DateTime.ParseExact(string.Format("{0}-{1}-{2} {3}", date1, date2.PadLeft(2, '0'), tyear, time), "MMM-dd-yyyy HH:mm", CultureInfo.GetCultureInfo("en-US"));
				}
				else if (date1.Length == 4)
				{
					date = DateTime.ParseExact(string.Format("{0}-{1}-{2} {3}", date1, date2.PadLeft(2, '0'), date3, time), "yyyy-MM-dd HH:mm", CultureInfo.GetCultureInfo("en-US"));
				}
